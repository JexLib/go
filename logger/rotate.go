...
func NewRotateWriter(root, prefix, suffix string) (*Writer, error) {
	l := &Writer{root: root, prefix: prefix, suffix: suffix, max: maxDefault, keep: keepDefault}
	if len(l.prefix) == 0 {
		l.prefix = "logger"
	}
...
func (r *Writer) openCurrent() error {
	cp := path.Join(r.root, r.prefix) + r.suffix
	var err error
...
func (r *Writer) rotate() error {
	...
	filename := fmt.Sprintf("%s_%d", r.prefix, time.Now().UnixNano()) + r.suffix
	if err := os.Rename(path.Join(r.root, r.prefix)+r.suffix, path.Join(r.root, filename)); err != nil {
		return err
	}
