...
//设置字符在图片中显示高度比例，默认0.8
func (c *Captcha) SetFontSize(percent float64) {
	c.charViewpercent = percent
}
...
func (c *Captcha) drawString(img *Image, str string) {
	...
	// 文字大小为图片高度的 0.8
	fsize := int(float64(c.size.Y) * c.charViewpercent)
	// 用于生成随机角度
